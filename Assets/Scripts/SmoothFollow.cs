...
{
    public float defaltFOV = 0.0f, desiredFOV = 0.0f;
    [Range(0,2)] public float smothTime = 0.0f;
    private bool chargeButtonIsCkicked;
    ...
    private void SpeedFollow()
    {
        if (chargeButtonIsCkicked)
        {
            Camera.main.fieldOfView = Mathf.Lerp(Camera.main.fieldOfView, desiredFOV, Time.deltaTime * smothTime);
        }
        else Camera.main.fieldOfView = Mathf.Lerp(Camera.main.fieldOfView, defaltFOV, Time.deltaTime * smothTime);

    }



    public void UpSpeedFollow()
    {
        chargeButtonIsCkicked = true;

    }

    public void DownSpeedFollow()
    {
        chargeButtonIsCkicked = false;
    }
